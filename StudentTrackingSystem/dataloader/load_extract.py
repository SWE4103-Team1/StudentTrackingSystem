...
class DataFileExtractor:
    ...
    def uploadPersonDataFile(self, personfile):
        dfp = pd.read_table(
            personfile, header=0, squeeze=True
    ...
    def makeStudent(self, dfp):
        if Student.objects.filter(student_number=dfp[0]).exists():
            return None
        else:
            student = Student(
    ...
    def IterCourses(self, dfc):
        ...
        try:
            courseList = list(
                filter(None, courseList)
            )  ##This filters out the None types in the list
        except ObjectDoesNotExist:
            pass  ## Error is thrown if there are No "None types"
        return courseList

    def makeCourse(self, dfc):
        if Course.objects.filter(course_code=dfc[3], section=dfc[8]).exists():
            return None
        else:
            course = Course(
                course_code=dfc[3],
    ...
    def makeEnrolments(self, dfe):
        if Enrolment.objects.filter(
            course=Course.objects.filter(course_code=dfe[3], section=dfe[8])[0],
            student=Student.objects.filter(student_number=dfe[0])[0],
        ).exists():
            return None
        else:
            enrolment = Enrolment(
    ...
    def makeTransferCourse(self, dft):
        if Course.objects.filter(
            course_code=dft[1], credit_hours=dft[3], upload_set=self.get_upload_set()
        ).exists():
            return None
        else:
            course = Course(
    ...
    def makeTransferEnrolments(self, dft):
        if Enrolment.objects.filter(
            course=Course.objects.get(course_code=dft[1], section="**"),
            ...
            enrolment = Enrolment(
                ...
                course=Course.objects.filter(course_code=dft[1], credit_hours=dft[3])[
                    0
                ],
                upload_set=self.get_upload_set(),
                term="Transfer",
